package com.csdn.netty.qq_18603599.client.tcp;

import com.alibaba.fastjson.JSONObject;
import com.csdn.netty.qq_18603599.bean.ClientRequest;
import com.csdn.netty.qq_18603599.bean.DefaultFuture;
import com.csdn.netty.qq_18603599.bean.Response;
import com.csdn.netty.qq_18603599.bean.User;
import com.csdn.netty.qq_18603599.initialzer.tcp.TcpClientInitalizer;
...
public class TcpNettyClient  {
    ...
    public static Object send(ClientRequest request){
        try{
            System.out.println("客户端向服务端发送请求数据:"+JSONObject.toJSONString(request));//客户端直接发送请求数据到服务端
            future.channel().writeAndFlush(JSONObject.toJSONString(request));//根据\r\n进行换行
            future.channel().writeAndFlush("\r\n");//通过请求实例化请求和响应之间的关系
            DefaultFuture defaultFuture = new DefaultFuture(request);//通过请求ID，获取对应的响应处理结果
    ...
    static  class UserRequestThread implements Runnable{
        ...
        public void run() {
            synchronized (UserRequestThread.class){
                ...
                request.setContent(user);//拿到请求的结果
